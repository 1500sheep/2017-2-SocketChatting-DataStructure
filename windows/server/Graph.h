...
public:
	...
	*/
	...
	//T값 의 edge관련된 vertex만 갖고 온다
	vector<T> PrintOneEdges(T vertex);

	vector<T> RecommandPath(T vertex);
private:
...
// Display all weights in the graph.
template<typename T>
vector<T> Graph<T>::PrintOneEdges(T vertex)
{
	vector<T> v_List;
	for (int i = 0; i < numVertices; i++)
	{
...
vector<T> Graph<T>::RecommandPath(T vertex)
{
	...
	if (!IsMarked(vertex))
	{
		...
		while (!vertexQueue.IsEmpty())
		{
			...
			GetToVertices(item, secondQueue);
			while (!secondQueue.IsEmpty()) {
				secondQueue.DeQueue(secondItem);
				temp_v.push_back(secondItem);
			}
		}
	}
	return temp_v;
